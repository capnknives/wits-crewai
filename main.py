...
try:
    ...
    print("[MAIN] VectorMemory module not found or import error. Vector search capabilities will be disabled if not using EnhancedMemory.")
except Exception as e_vm_import:
    print(f"[MAIN] Error importing VectorMemory: {e_vm_import}. Vector search capabilities will be disabled.")


from ethics.ethics_rules import EthicsFilter, EthicsViolation
from core.voice import get_voice_input # Using your updated voice module
from core.router import route_task
from core.message_bus import MessageBus, MessageBusClient # MessageBus kept if used by Planner
...
from agents.tools.weather_tool import WeatherTool # New tool
from agents.tools.data_visualization_tool import DataVisualizationTool # New tool
from agents.tools.pdf_generator_tool import PdfGeneratorTool # New tool
...
def main():
    ...
    # Load configuration
    config: Dict[str, Any] = {} # Type hint for config
    ...
    config.setdefault("web_interface", {
        "enabled": config.get("web_interface", {}).get("enabled", False),
        "port": 5000, "host": "0.0.0.0", "debug": True,
        "enable_file_uploads": True, "max_file_size": 5
    })
    config.setdefault("autonomous_planner", {
        "enabled": True,
    ...
    memory: Union[EnhancedMemory, VectorMemory]
    if VECTOR_MEMORY_AVAILABLE:
        ...
        except Exception as e_vm_init_main:
            print(f"[SYSTEM_ERROR] Vector Memory instantiation failed in main.py ({e_vm_init_main}), falling back to Enhanced Memory.")
            memory = EnhancedMemory(memory_file='enhanced_memory.json')
            print("[SYSTEM] Using Enhanced Memory system (VectorMemory fallback).")
    else:
        memory = EnhancedMemory(memory_file='enhanced_memory.json')
        print("[SYSTEM] Using Enhanced Memory system (VectorMemory not available).")


    # Initialize Message Bus
    message_bus = MessageBus(save_path='message_history.json') # Main message bus
    ...
    weather_tool = WeatherTool(api_key=config.get("weather_api_key")) # Pass API key if configured
    ...
    engineer_tools = common_tools + [write_file_tool] # Engineer might also benefit from read_file
    scribe_tools = common_tools + [write_file_tool, pdf_generator_tool]
    planner_tools = common_tools # Planner might need more tools later, or delegate tool use
    ...
    agents_for_planner: Dict[str, Any] = {
        "scribe": scribe_agent, "engineer": engineer_agent,
        "analyst": analyst_agent, "researcher": researcher_agent,
        "quartermaster": quartermaster # Planner can delegate to Quartermaster for file/goal info
    }
    planner_mb_client = MessageBusClient("planner_main", message_bus) if message_bus else None
    ...
    if use_voice:
        try:
            # Ensure sounddevice defaults are set
            sd.default.samplerate = 16000
            sd.default.channels = 1
            sd.default.dtype = 'float32'
            # Model loading is handled by get_voice_input now
            print(f"[VOICE] Voice input enabled. Whisper model '{config.get('whisper_model')}' will be used via get_voice_input.")
    ...
    while True:
        ...
        try:
            ...
            if processed_user_command_this_iteration and user_input_text:
                ...
                if agent_name in agents:
                    ...
                    if goal_ref_match:
                        try:
                            ...
                            # Get goals sorted as they are displayed by list_goals
                            temp_goals_list = memory.get_goals_list() # This gets active goals
                ...
                try:
                    if agent_name in agents:
                        ...
                        if isinstance(agent_instance, PlannerAgent):
                            # PlannerAgent's run method expects 'command' as the first positional argument
                            # or as a named argument if others are present.
                            # For direct user commands, associated_goal_id_for_new_plan is typically None.
                            result_text = agent_instance.run(
                                command=task_content,
                                associated_goal_id_for_new_plan=None
                            )
                        else:
                            # Most other agents expect the task content as the first positional argument
                            result_text = agent_instance.run(task_content)
                        # END MODIFIED AGENT CALL
                ...
                print(f"\n{result_text}\n")
                last_autonomous_check_time = time.time() # Reset timer after user interaction
            ...
            if autonomous_enabled and \
               (not processed_user_command_this_iteration or \
                ...
                pending_goals = memory.get_pending_goals() # This should return List[Dict]
                ...
                if pending_goals:
                    ...
                    if goal_to_process_autonomously:
                        ...
                        # Construct the command for the PlannerAgent
                        # The PlannerAgent's run method expects the goal description as the main command
                        ...
                        try:
                            ...
                            # PlannerAgent's run method signature: run(self, command: str, associated_goal_id_for_new_plan: Optional[str] = None)
                            planner_execution_result = planner_agent.run(
                                command=planner_autonomous_command, # This is the full command string for the planner
                            ...
                            # Check the final status of the goal in memory
                            ...
                            if final_goal_obj:
                                ...
                                # This case might occur if the goal was completed and moved to completed_goals
                                print(f"[AUTONOMOUS_PLANNER] Goal ID {goal_id_auto[:8]}... processing finished (likely completed or moved).")
