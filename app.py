...
import json
import time
import threading # Kept for process_task_queue
import queue     # Kept for task_queue
import yaml
import sys
from datetime import datetime
import re
from werkzeug.utils import secure_filename # For file uploads
from typing import List, Dict, Any, Optional, Tuple, Union # ADDED THIS LINE

# Add the parent directory to the path so we can import from the WITS modules
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

# Import WITS components
# Assuming core.memory.Memory is a basic fallback, but EnhancedMemory is preferred
from core.enhanced_memory import EnhancedMemory
VECTOR_MEMORY_AVAILABLE = False # Default, will be updated
try:
    from core.vector_memory import VectorMemory
    VECTOR_MEMORY_AVAILABLE = True
except ImportError:
    print("[WEB_APP] VectorMemory module not found or import error. Vector search capabilities will be disabled if not using EnhancedMemory.")
except Exception as e_vm_import:
    print(f"[WEB_APP] Error importing VectorMemory: {e_vm_import}. Vector search capabilities will be disabled.")


from ethics.ethics_rules import EthicsFilter, EthicsViolation
from core.router import route_task
from core.message_bus import MessageBus, MessageBusClient, Message # MessageBus kept if used
...
# --- Global Variables for Task Processing (from user's app.py) ---
task_queue = queue.Queue() # Renamed from task_queue_web for consistency if only one queue
task_results: Dict[str, Any] = {}  # Added type hint
task_status: Dict[str, str] = {}   # Added type hint
command_history: List[Dict[str, Any]] = []  # Added type hint

# --- Configuration Loading ---
config: Dict[str, Any] = {} # Added type hint
config_path = "config.yaml" # Ensure this path is correct relative to app.py execution
...
# Apply defaults robustly
config.setdefault("internet_access", True)
config.setdefault("voice_input", False) # app.py doesn't handle voice directly
config.setdefault("allow_code_execution", False)
config.setdefault("output_directory", "output")
os.makedirs(config.get("output_directory", "output"), exist_ok=True) # Ensure output_directory is a string
...
config.setdefault("web_interface", { # Settings for Flask app itself
    "port": 5000, "host": "0.0.0.0", "debug": True,
    "enable_file_uploads": True, "max_file_size": 5 
})
# Note: Autonomous planner settings from config are used by main.py, not directly by app.py's task loop

# --- Initialize Memory System ---
memory: Optional[EnhancedMemory] = None # Type hint for memory
if VECTOR_MEMORY_AVAILABLE:
    ...
    memory = EnhancedMemory(memory_file='enhanced_memory.json')
    print("[WEB_APP] Using Enhanced Memory system for app.py (VectorMemory not available).")
...
else: # Fallback if QM not initialized
...
# Agent instances for app.py's task processor
...
# Filter out None instances if initialization failed
...
def process_task_queue_thread_target():
    ...
    while True:
        task_id_local, agent_key_routed_local, task_content_local = "", "", "" # Define outside try for finally
        try:
            ...
            if agent_instance_to_run:
                ...
                result_from_agent = agent_instance_to_run.run(task_content_local, associated_goal_id_for_new_plan=assoc_goal_id)
            ...
            error_msg = f"[Agent Execution Error - {agent_key_routed_local}] {str(e_proc)}"
            task_results[task_id_local] = {"agent": agent_key_routed_local, "command": task_content_local, 
                                           "result": f"{error_msg}\nTraceback:\n{error_details}", 
            ...
            # Ensure task_done is called even if an error occurs before agent_instance_to_run.run()
            if task_id_local: # Check if task_id_local was assigned
                 task_queue.task_done()


app_task_processor_thread = threading.Thread(target=process_task_queue_thread_target, daemon=True)
if not app.debug or os.environ.get('WERKZEUG_RUN_MAIN') == 'true': # Avoid starting thread twice in debug mode
...
def parse_goals_for_template(goals_text_list_str: str) -> List[Dict[str, str]]:
    """Parses the string output of memory.list_goals() into a list of dicts for the template."""
    parsed_goals = []
    if not memory: 
    ...
    if goals_text_list_str and "No current goals" not in goals_text_list_str:
        ...
        for line_content in lines:
            ...
            match = re.match(
                r"^\d+\.\s+ID:\s+([a-f0-9]{8}).*?\s+Task:\s+(.+?)\s+(?:\[Status:\s*([\w\s]+)\])", # Allow space in status
                line_content, 
                re.IGNORECASE
            )
            if match:
                goal_id_short = match.group(1)
                ...
                if isinstance(raw_goals, list):
                    for g_dict in raw_goals:
                        if isinstance(g_dict, dict) and g_dict.get('id','').startswith(goal_id_short) and g_dict.get('task') == task_desc:
                            full_goal_id = g_dict.get('id')
                            break
                
                parsed_goals.append({
                    "id": full_goal_id or goal_id_short,
...
def index_route():
    ...
    files_for_ui = []
    try:
        files_output_str = quartermaster_for_app.list_files() # Use app's QM
...
def submit_task_api():
    ...
    # Ensure "complete goal by id" is correctly routed and content is set for Quartermaster
    if "quartermaster" in command_from_user.lower() and "complete goal by id" in command_from_user.lower():
        ...
        # Extract the "complete goal by id ..." part for Quartermaster
        ...
        if match_complete_cmd:
            # Reconstruct the command for Quartermaster to parse
        ...
        else: # Fallback if specific regex fails, pass the relevant part
            idx = command_from_user.lower().find("complete goal by id")
            if idx != -1:
                task_content_for_agent = command_from_user[idx:]
            # If still not good, router's original task_content_for_agent will be used.
...
def get_task_status_api(task_id: str): # Added type hint
...
def get_file_content_api(filename: str): # Added type hint
    ...
    try:
        ...
        # Ensure filename is just the name, not a path trying to escape output_dir
        safe_filename = secure_filename(filename)
        if safe_filename != filename: # Or if filename contains '..' or starts with '/'
            print(f"[WEB_APP_SECURITY] Potentially unsafe filename in get_file_content_api: '{filename}', sanitized to '{safe_filename}'")
            # Depending on policy, either reject or use sanitized. Rejecting is safer.
            return jsonify({"status": "error", "message": "Invalid filename."}), 400
        
        # Path for Sentinel check should be relative to output dir if QM expects that
        ...
        # Quartermaster's read_file should handle resolving this to the full path within its output_dir
...
def serve_file_from_output_route(filename: str): # Added type hint
    ...
    # Security: Prevent path traversal. send_from_directory handles some of this, but an explicit check is good.
...
def goals_api_route():
    ...
    if completed_text_list and "No completed goals yet" not in completed_text_list:
        ...
        if lines and lines[0].strip().startswith("Recently Completed Goals:"): lines = lines[1:]
        for line_content in lines:
            completed_goals_parsed.append({"display_text": line_content.strip()})
    ...
    if file_to_upload:
        ...
        try:
            if sentinel_agent_for_app:
                 sentinel_agent_for_app.approve_action("WebAppUpload", "file_write", detail=save_path) # Check full path
...
def agent_view_route(agent_name_route_param: str): # Added type hint
    ...
    if request.method == 'POST':
        ...
        for key in ["internet_access", "ethics_enabled", "allow_code_execution", "output_directory", "voice_input"]:
            if key in data_from_request:
                # Basic type conversion for safety
                if isinstance(config.get(key), bool):
                    config[key] = bool(data_from_request[key])
                elif isinstance(config.get(key), int) and key not in ["port", "max_file_size", "voice_input_duration"]: # port/size handled by web_interface
        ...
        # Voice specific settings (already covered by system_keys if "voice_input" is there)
...
if __name__ == '__main__':
    ...
    web_run_config = config.get("web_interface", {}) # Ensure config is loaded before this
    app.run(
        host=web_run_config.get("host", "0.0.0.0"),
        port=web_run_config.get("port", 5000),
        debug=web_run_config.get("debug", True) # Flask debug mode
    )
